...
**Следующая задача:** 1.4 (Google Sign In) — Google Client ID получен
...
_12.03.2026 — Задачи 1.1 и 1.2 выполнены (код в репо). Настроена рабочая среда: Xcode 15.2 + iOS 17.2 симулятор, Flutter 3.22.3 (ограничение macOS 13), CocoaPods через rbenv Ruby 3.2.2. Shell — bash, PATH в ~/.bash_profile. Google Cloud Console создан, OAuth Client ID получен. Документы MASTER.md и STEP-BY-STEP.md обновлены под монорепо. Следующая задача: 1.4 (Google Sign In)._
