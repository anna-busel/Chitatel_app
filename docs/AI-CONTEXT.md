...
**Следующая задача:** 0.6 (Xcode проект Flutter) → потом 1.5 (Flutter — дизайн-система)
...
| 0.6 | Xcode проект Flutter | Среда готова, проект ещё не создан | СЛЕДУЮЩАЯ ЗАДАЧА |
...
_15.03.2026 — Установлены Node.js 20.20.1 (nvm) и MongoDB 7.0.20 (бинарники). Протестирован весь бэкенд auth локально: health, register, login, google (с невалидным токеном — корректная ошибка). Исправлен баг с дублированными индексами в User.js. Бэкенд auth (задачи 1.1, 1.2, 1.4) полностью рабочий. Следующая задача: 0.6 (Xcode проект Flutter) → 1.5 (дизайн-система)._
