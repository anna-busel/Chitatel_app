...
**Блокеры:** 1.3 (Apple Sign In) ждёт покупки Apple Dev Account. 0.6 (Xcode проект Flutter) нужен перед 1.5.

---

## ПРОГРЕСС

**Готовые задачи:** 0.5, 1.1, 1.2, 1.4
...
- **Node.js:** установить локально для тестирования бэкенда (ещё не сделано)
- **MongoDB:** установить локально для тестирования (ещё не сделано)
...
| 0.6 | Xcode проект Flutter | Среда готова, проект ещё не создан | Перед задачей 1.5 |
...
_12.03.2026 — Задача 1.4 выполнена: Google Sign In бэкенд. Создан google-auth.service.js (верификация idToken через google-auth-library, поиск/создание User по googleUserId, привязка Google к существующему email-аккаунту). Обновлён routes/auth.js — добавлен POST /api/auth/google с Zod-валидацией. Добавлена зависимость google-auth-library. Следующая задача: 1.5 (Flutter — дизайн-система), но требует 0.6 (Xcode проект Flutter)._
