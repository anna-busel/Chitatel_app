...
- [x] D4. Пройти все 33 экрана по списку из A8, починить найденное. **Выполнено 12–17.09.2026.** Исправлено всё найденное; ждёт подтверждения в новой сборке. Отдельно: заглушка «Фото не открылось» вместо невидимого значка. Старые HEIC-файлы на диске — отдельная задача (разовый скрипт).
