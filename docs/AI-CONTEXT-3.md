...
**🔧 ЧАТ — ИДЁТ ОТЛАДКА ЛАГА И СКРОЛЛА (эксперименты, см. раздел 5 сессии):** эксперимент 1 (`85c02c9`, проверен билдом) не помог; эксперимент 2 (`2829ece`) — текстура бумаги УБРАНА, автоскролл после отправки починен (найдена настоящая причина — ранний return при WS-эхе). Ждём билд и вердикт.
...
**⏭️ СЛЕДУЮЩЕЕ:** проверки последнего билда (чат: скролл после отправки + вязкость без текстуры; картинки чата; плашка у сезонницы; голосовые под Анной) → скрипт 64 разборов (ждёт: роль ключа UN4ZB8T93H App Manager?, скрины юзера, картинки).

**ПРИМЕНИТЬ:** сервер `git pull` + `pm2 restart` (plan в clubAccess, фикс home.js); билд Codemagic (чат: `2829ece`).
...
- Вывод: гипотеза GlobalKey не подтверждена (совпадает с замерами в статьях — ключи почти не влияют на скролл). Ключи назад НЕ возвращаем (без них проще, поведение перехода приемлемое).

**ЭКСПЕРИМЕНТ 2 — `2829ece` (12.07, ждёт билда).**
- **(а) Автоскролл — найдена НАСТОЯЩАЯ причина, это баг, а не «недоезд анимации»:** в `_insertOwnMessage` первой строкой стоял ранний `return` при дубле id. WS-эхо своего сообщения часто приходит РАНЬШЕ ответа POST → сообщение уже в ленте → `return` → `_scrollToBottom()` не вызывался ВООБЩЕ. Поэтому правки скролла из эксперимента 1 не могли сработать — их просто не выполняли. Фикс: дедупликация касается только ВСТАВКИ, скролл вниз выполняется всегда для своего сообщения (и в ветке WS-эха тоже, если сообщение своё); плюс вторая попытка следующим кадром (высота картинки/многострочного текста известна только после layout — первый прыжок мог «не доехать»); цель скролла — `position.minScrollExtent`.
- **(б) Текстура бумаги УБРАНА** (`_PaperTexturePainter` и `_paperDotOpacity` удалены; фон чата — чистый `AppColors.background`). Ранее считалась «вне подозрений» (RepaintBoundary + shouldRepaint=false), но она рисует десятки тысяч `drawCircle` шагом 3px на полноэкранном слое, который перерисовывается при каждом изменении размера вьюпорта (клавиатура, плашки). Убрана по решению юзера — снимаем подозрение полностью.
...
1. ✅ Закреп кофейный `b3d4c4e`. 2. ✅ Скругления 18px + хвостики + компактные реакции `9d9d04c`. 3. ✅ Бейдж «Автор клуба» + полоска `34f1824`+`b0eca94`. 4. ✅ Текстура бумаги (CSS-точки CustomPainter `_PaperTexturePainter`, `_paperDotOpacity=0.08`, RepaintBoundary) `649d32a` — **[12.07: УБРАНА, `2829ece`, при отладке чат-лага]**. 5. ✅ Разделители дат `1215f7e9`. 6. ✅ Аватар-серия `1215f7e9`.
**ОТМЕНЕНО:** галочки прочтения; кофейный в пузырях; книжный паттерн; смена базового фона.
**[11-12.07: chat_message_bubble и chat_tab переписаны оптимизацией — тень→рамка, foregroundDecoration, RepaintBoundary на элементы, cacheExtent; GlobalKey убраны; текстура удалена; опечатка «применялall» исправлена попутно.]**
...
🔧 ЧАТ-ЛАГ+СКРОЛЛ [11-12.07]: базовая оптимизация (тень→рамка, RepaintBoundary, cacheExtent, клипы,
   Impeller возвращён); ЭКСП.1 85c02c9 (GlobalKey убраны) — ОТРИЦАТЕЛЬНЫЙ, лаг остался, скролл не работал;
   ЭКСП.2 2829ece — текстура убрана + автоскролл починен (ранний return при WS-эхе). ЖДЁМ БИЛД.
ПРИМЕНИТЬ: сервер pull+pm2 restart; билд Codemagic (2829ece).
СЛЕДУЮЩЕЕ (по одной, СПРАШИВАТЬ перед кодом):
  1. Проверки билда: чат — скролл после отправки + вязкость без текстуры (вердикт!), картинки чата;
     плашка у сезонницы; обложка на главной; голосовые под Анной.
...
**#54 ⚠️ ПЕРФ-ОПТИМИЗАЦИЯ СПИСКОВ — СНАЧАЛА ДОРОГИЕ ВЕЩИ ПО ЧЕК-ЛИСТУ (12.07).** Тени (blur) на элементах, clip antiAlias, отсутствие RepaintBoundary, анимационная обвязка «на всякий случай» — официально самое дорогое в лентах. GlobalKey в списке — вопреки интуиции — по замерам почти не влияет на скролл (подтверждено экспериментом 1 `85c02c9`: ключи убраны — лаг остался). Дёрганый скролл лент с картинками — многолетняя болячка Flutter (issue #61193), потолок ниже нативного; сравнение с Telegram некорректно.

**#55 ⚠️⚠️ «НЕ СКРОЛЛИТ ВНИЗ» — ИЩИ РАННИЙ RETURN, А НЕ КРУТИ АНИМАЦИЮ (12.07).** Два билда правили сам скролл (animateTo→jumpTo), а вызова скролла не происходило вовсе: `_insertOwnMessage` делал `return` при дубле id, а дубль возникал штатно — WS-эхо своего сообщения приходит раньше ответа POST. Правило: дедупликация должна закрывать только ВСТАВКУ в список, а побочные эффекты (скролл, фокус, звук) — выполняться независимо. И прежде чем тюнить анимацию — убедиться логом/чтением кода, что функция вообще вызывается (перекликается с уроком #40).

---

*Обновлён 12.07.2026 (сессия 09-12.07). **✅ Платёжный контур подписок ЗАКРЫТ живыми покупками** (апгрейд Flutter 3.44.6+Xcode 26.4 починил verify; monthly+season куплены; продление/отмена/restore/переходы ✅; авто-restore убран `3cd0c1d`, success_screen удалён из потока). **✅ Сезоны** (окна+тексты «по цене двух»+карточка/анонс/плашка/полоска+тест-ручка тройной тап — убрать в Фазе 7). **✅ Фикс главной** `a7964f2`. **✅ plan в clubAccess**. **🔧 Чат:** эксперимент 1 `85c02c9` (GlobalKey убраны) — отрицательный (лаг остался, автоскролл не работал); эксперимент 2 `2829ece` — текстура бумаги УБРАНА + автоскролл починен (настоящая причина: ранний return при WS-эхе, урок #55) — ждём билд. **⚠️ Тестовая среда:** TestFlight ≠ sandbox (урок #51). **Следующее:** проверки билда → скрипт 64 разборов. Прогресс фиксировать далее ТОЛЬКО здесь.*
