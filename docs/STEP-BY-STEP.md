...
### Задача 0.5: GitHub репозитории ⏱ 1ч
**Что делаем:** Создать репозитории + структуру
**Действия:**
1. Создать GitHub репо: `chitatel-api` (бэкенд) — приватный
2. Создать GitHub репо: `chitatel-app` (Flutter) — приватный
3. В `chitatel-api`:
   ```
   docs/
     MASTER.md          ← копируем наш документ
     AI-CONTEXT.md      ← создаём (шаблон ниже)
     prototype-v4_2.jsx   ← копируем прототип
   src/
   package.json
   .eslintrc.json
   .env.example
...
4. В `chitatel-app`:
   ```
   docs/
     AI-CONTEXT.md      ← ссылка на тот же формат
   lib/
   pubspec.yaml
   ios/
   ```
5. Начальный AI-CONTEXT.md:
   ```markdown
   # AI-CONTEXT — ЧИТАТЕЛЬ
   ...
   ## Проблемы: нет
   ```
**Результат:** Два репо с начальной структурой
...
**Действия:**
1. `flutter create --org app.chitatel chitatel_app`
...
**Файлы:** `admin/` (отдельная папка в репо `chitatel-api`)
**Действия:**
