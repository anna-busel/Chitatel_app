 ...
 * `createdAt = now`. Двадцать сообщений, упавших подряд за десять минут,
...
const CURRENT_MESSAGES = [
  {
    ...
    text:
      ...
      'Разбор выходит частями, новая часть — по понедельникам. Не переживайте, если не успеваете за ритмом: чат живёт весь месяц, возвращайтесь когда будет время и силы.\n\n' +
      ...
      'Пишите сюда мысли, несогласия, выписанные цитаты. Спорить со мной можно и нужно — из спора обычно и рождается самое интересное.',
  },
  {
    who: 'marina',
    at: 0.04,
...

  {
    ...
    at: 0.2,
    text:
      'Дочитала первую часть. Сцена, где Настасья Филипповна бросает пачку денег в камин, — у меня руки тряслись. Причём не от неё, а от того, как все на это смотрят.',
    reactions: { '🔥': ['marina'] },
  },
  {
    who: 'marina',
  ...
  },
  {
    ...
    text:
      ...
      'Достоевский, по-моему, не отвечает, хорошо это или плохо. Он просто показывает цену. В разборе я к этому вернусь во второй части.',
...

  {
    ...
    at: 0.88,
    text:
      'Вторую часть слушала в дороге. Момент про сострадание и жалость — что это вообще-то разные вещи — переслушала дважды.',
  },
  {
    who: 'anna',
    at: 0.95,
    text:
      'Следующая часть выйдет в понедельник. Там будем разбирать Рогожина, и я почти уверена, что кто-нибудь со мной опять не согласится.',
    reactions: { '🔥': ['marina'] },
  },
];
...
const ARCHIVE_MESSAGES = [
  ...
  },
  {
    ...
    at: 0.8,
    text:
      'На этом закрываем месяц. Спасибо всем, кто читал и писал сюда, — с вами было хорошо.\n\n' +
      'Про последнюю сцену: мне кажется, её и не надо «понимать». Стейнбек заканчивает жестом, а не выводом, и правильно делает. Любой вывод был бы мельче.\n\n' +
      'Чат остаётся открытым, возвращайтесь если захочется дописать.',
    reactions: { '❤️': ['marina', 'darya'] },
  },
];
...
async function main() {
  ...
  if (KSENIA_EMAIL) {
    ...
    console.log('ℹ️  --ksenia не указан: четыре её реплики пропускаются');
  }
