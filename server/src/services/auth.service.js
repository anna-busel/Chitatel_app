...
const logout = async ({ refreshToken, userId }) => {
  ...
  // Push-токены устройств снимаем: после выхода пуши этому юзеру на этот
  // телефон не нужны (иначе следующий вошедший на устройстве получит чужие).
  // Снимаем ВСЕ устройства, а не одно: клиент при выходе не сообщает, с
  // какого именно телефона вышли. Оставшиеся устройства вернут свой токен
  // сами — push_service.dart регистрирует его при каждом запуске приложения.
  // 09.09.2026 (C1): к устаревшему pushToken добавлен список devices.
