...
function computeClubAccess(user, club, now = new Date()) {
  ...
  // БОНУС-МЕСЯЦ: клуб, который человек ОПЛАТИЛ, остаётся полностью рабочим
  // весь следующий календарный месяц — чат, Q&A, разбор. Оплатил сентябрь →
  // в октябре сентябрьский клуб живой; продлил или нет, значения не имеет,
  // он за этот клуб заплатил.
  //
  // ⛔️ 17.08.2026 — проверка isPaidClub здесь ОБЯЗАТЕЛЬНА, не убирать.
  // Раньше тут стоял everSubscribed (любой, кто когда-либо платил), без
  // сверки с оплаченным набором. Из-за этого человек, впервые оплативший
  // сентябрь, забирал АВГУСТОВСКИЙ клуб даром — и чат, и разбор. Это тот же
  // класс бага, что чинили 30.07 для текущего клуба, только в другую сторону.
  // Клуб, которого нет в clubMonthsEntitled, не открывается никогда.
  if (isInArchiveWindow && isPaidClub) {
    return {
      kind: 'archive',
      tier: user.subscriptionStatus,
      plan,
 ...
 *
 * Архив (бонус-месяц) работает по ТОМУ ЖЕ набору: клуб из clubMonthsEntitled
 * остаётся полностью доступен (чат, Q&A, разбор, canPost=true) весь следующий
 * календарный месяц. Продление на это не влияет — оплатил клуб, значит месяц
 * хвоста твой. Клуб НЕ из набора не открывается ни как архив, ни как текущий.
 *
 ...
 *      - клуб в оплаченном наборе → kind='archive', canPost (активная подписка
 *        НЕ требуется: бонус-месяц положен и тому, кто не продлил)
 *      - клуб не оплачивался → 403 SUBSCRIPTION_REQUIRED
